// JavaScript Documentvar BMapLib=window.BMapLib=BMapLib|| {
}
;
...
;
	...
	var n=function(q) {
	o.call(j,q)}
,i=e.event._listeners,l=e.event._eventFilter,p,k=m;
	m=m.toLowerCase();
	if(l&&l[m]) {
	p=l[m](j,m,n);
	k=p.type;
...
;
	...
	var q=e.event._listeners,l=q.length,m=!j,p,o,i;
	while(l--) {
	p=q[l];
...
;
	...
	var l=document.createElement(k),j=i|| {
}
;
...
document.cookie=k+"="+l+(j.path?";
	path="+j.path:"")+(i?";
	expires="+i.toGMTString():"")+(j.domain?";
	domain="+j.domain:"")+(j.secure?";
	secure":"")}
;
...
,e.object.extend(a.prototype, {
	...
	var j=BMapLib.SearchInfoWindow.instance,i=j.length;
	while(i--) {
	if(j[i]._isOpen) {
	j[i].close()}
}
...
,_panBox:function() {
	...
	return}
var m=parseInt(this._map.getContainer().offsetHeight,10),r=parseInt(this._map.getContainer().offsetWidth,10),n=this._boxHeight,i=this._boxWidth;
	if(n>=m||i>=r) {
	return}
if(!this._map.getBounds().containsPoint(this._point)) {
	this._map.setCenter(this._point)}
var j=this._map.pointToPixel(this._point),s,p,l=i/2-28-j.x+10,q=i/2+28+j.x-r+10;
	if(this._marker) {
...
,_createSearchTemplate:function() {
	...
	"class":"BMapLib_SearchInfoWindow",id:"BMapLib_SearchInfoWindow"+this.guid}
);
	var i=['<div class="BMapLib_bubble_top">','<div class="BMapLib_bubble_title" id="BMapLib_bubble_title'+this.guid+'"></div>','<div class="BMapLib_bubble_tools">','<div class="BMapLib_bubble_close" title="关闭" id="BMapLib_bubble_close'+this.guid+'">',"</div>",'<div class="BMapLib_sendToPhone" title="发送到手机" id="BMapLib_sendToPhone'+this.guid+'">',"</div>","</div>","</div>",'<div class="BMapLib_bubble_center">','<div class="BMapLib_bubble_content" id="BMapLib_bubble_content'+this.guid+'">',"</div>",'<div class="BMapLib_nav" id="BMapLib_nav'+this.guid+'">','<ul class="BMapLib_nav_tab" id="BMapLib_nav_tab'+this.guid+'">','<li class="BMapLib_first BMapLib_current" id="BMapLib_tab_search'+this.guid+'" style="display:block;
	">','<span class="BMapLib_icon BMapLib_icon_nbs"></span>在附近找',"</li>",'<li class="" id="BMapLib_tab_tohere'+this.guid+'" style="display:block;
	">','<span class="BMapLib_icon BMapLib_icon_tohere"></span>到这里去',"</li>",'<li class="" id="BMapLib_tab_fromhere'+this.guid+'" style="display:block;
	">','<span class="BMapLib_icon BMapLib_icon_fromhere"></span>从这里出发',"</li>","</ul>",'<ul class="BMapLib_nav_tab_content">','<li id="BMapLib_searchBox'+this.guid+'">','<table width="100%" align="center" border=0 cellpadding=0 cellspacing=0>','<tr><td style="padding-left:8px;
	"><input id="BMapLib_search_text'+this.guid+'" class="BMapLib_search_text" type="text" maxlength="100" autocomplete="off"></td><td width="55" style="padding-left:7px;
	"><input id="BMapLib_search_nb_btn'+this.guid+'" type="submit" value="搜索" class="iw_bt"></td></tr>',"</table>","</li>",'<li id="BMapLib_transBox'+this.guid+'" style="display:none">','<table width="100%" align="center" border=0 cellpadding=0 cellspacing=0>','<tr><td width="30" style="padding-left:8px;
	"><div id="BMapLib_stationText'+this.guid+'">起点</div></td><td><input id="BMapLib_trans_text'+this.guid+'" class="BMapLib_trans_text" type="text" maxlength="100" autocomplete="off"></td><td width="106" style="padding-left:7px;
	"><input id="BMapLib_search_bus_btn'+this.guid+'" type="button" value="公交" class="iw_bt" style="margin-right:5px;
	"><input id="BMapLib_search_drive_btn'+this.guid+'" type="button" class="iw_bt" value="驾车"></td></tr>',"</table>","</li>","</ul>","</div>","</div>",'<div class="BMapLib_bubble_bottom"></div>','<img src="iw_tail.png"/*tpa=http://sdyypc.com/images/iw_tail.png*/ width="58" height="31" alt="" class="BMapLib_trans" id="BMapLib_trans'+this.guid+'" style="left:144px;
	"/>'];
	j.innerHTML=i.join("");
	this._div=j}
return this._div}
...
,_initDom:function() {
	if(!this.dom) {
	this.dom= {
	container:e.g("BMapLib_SearchInfoWindow"+this.guid),content:e.g("BMapLib_bubble_content"+this.guid),title:e.g("BMapLib_bubble_title"+this.guid),closeBtn:e.g("BMapLib_bubble_close"+this.guid),transIco:e.g("BMapLib_trans"+this.guid),navBox:e.g("BMapLib_nav"+this.guid),navTab:e.g("BMapLib_nav_tab"+this.guid),seartTab:e.g("BMapLib_tab_search"+this.guid),tohereTab:e.g("BMapLib_tab_tohere"+this.guid),fromhereTab:e.g("BMapLib_tab_fromhere"+this.guid),searchBox:e.g("BMapLib_searchBox"+this.guid),transBox:e.g("BMapLib_transBox"+this.guid),stationText:e.g("BMapLib_stationText"+this.guid),nbBtn:e.g("BMapLib_search_nb_btn"+this.guid),busBtn:e.g("BMapLib_search_bus_btn"+this.guid),driveBtn:e.g("BMapLib_search_drive_btn"+this.guid),searchText:e.g("BMapLib_search_text"+this.guid),transText:e.g("BMapLib_trans_text"+this.guid),sendToPhoneBtn:e.g("BMapLib_sendToPhone"+this.guid)}
;
...
if(!this.localSearch) {
	...
	renderOptions:i,onSearchComplete:function(l) {
	j._clearAddress();
	j._drawCircleBound()}
}
)}
...
,_showTabContent:function(m) {
	...
	var l=this.dom.navTab.getElementsByTagName("li"),j=l.length;
	for(var k=0,j=l.length;
	k<j;
	k++) {
	l[k].className=""}
switch(m) {
	case BMAPLIB_TAB_SEARCH:this.dom.seartTab.className="BMapLib_current";
	this.dom.searchBox.style.display="block";
	this.dom.transBox.style.display="none";
	break;
	case BMAPLIB_TAB_TO_HERE:this.dom.tohereTab.className="BMapLib_current";
	...
	break;
	case BMAPLIB_TAB_FROM_HERE:this.dom.fromhereTab.className="BMapLib_current";
	this.dom.searchBox.style.display="none";
	this.dom.transBox.style.display="block";
...
,_autoCompleteIni:function() {
	...
	input:this.dom.searchText,location:this._map}
);
	this.transAC=new BMap.Autocomplete( {
	input:this.dom.transText,location:this._map}
)}
...
,_drawCircleBound:function() {
	...
	strokeWeight:3,strokeOpacity:0.4,strokeColor:"#e00",filColor:"#00e",fillOpacity:0.4}
);
...
,_transitRouteComplete:function(i,l) {
	...
	var k=l.getStartStatus(),m=l.getEndStatus(),n="";
	n="找不到相关的线路";
...
this.lsAddress.setInfoHtmlSetCallback(function(p,o) {
	...
	border-radius:2px;
	background-image:-webkit-gradient(linear,left top,left bottom,from(#FFFDF8),to(#FFEECC))";
	n.innerHTML="设为"+l;
	o.appendChild(n);
...
,_setSearchTypes:function() {
	var p=this._unique(this._opts._searchTypes),k=this.dom.navTab,o=[this.dom.seartTab,this.dom.tohereTab,this.dom.fromhereTab],n=0,j=0,m=0,q;
...
this._adjustTransPosition()}
,_unique:function(l) {
	var k=l.length,j=l.slice(0),n,m;
	while(--k>=0) {
	m=j[k];
	if(m<0||m>2) {
	j.splice(k,1);
...
var k=this.container.getElementsByTagName("IMG");
	...
	continue}
k[j].style.cssText+=";
	FILTER:progid:DXImageTransform.Microsoft.AlphaImageLoader(src="+k[j].src+",sizingMethod=crop)";
	k[j].src="blank-1.gif"/*tpa=http://api.map.baidu.com/images/blank.gif*/}
}
...
e.on(l,"mousedown",i);
	...
	z-index:99;
	";
	l.innerHTML='<input type="text" value="'+j._radius+'" style="width:30px;
	" id="BMapLib_search_radius'+this.guid+'"/>m <a href="javascript:void(0)" title="修改" onclick="BMapLib.SearchInfoWindow.instance['+this.guid+']._changeSearchRadius()" style="font-size:12px;
	text-decoration:none;
	color:blue;
	">修改</a><img src="iw_close1d3.gif"/*tpa=http://api.map.baidu.com/images/iw_close1d3.gif*/ alt="关闭" title="关闭" style="cursor:pointer;
	padding:0 5px;
	" onclick="BMapLib.SearchInfoWindow.instance['+this.guid+']._closeCircleBound()"/>';
...
,generalDom:function() {
	var m=document.createElement("div"),j=this._map.getSize(),l=0,k=0;
	if(j.width>450) {
	k=(j.width-450)/2}
if(j.height>260) {
	l=(j.height-260)/2}
m.style.cssText="position:absolute;
	...
	ovflow:hidden;
	";
	var i=['<div class="BMapLib_sms_tab_container">',"<span>发送到手机</span>",'<span id="BMapLib_sms_tip" style="display:none;
	">',"</span>","</div>",'<div id="BMapLib_sms_pnl_phone" class="BMapLib_sms_pnl_phone" style="display:block;
	">','<div class="BMapLib_ap" id="pnl_phone_left" style="display:block;
	">',"<table>","<tr>","<th>发送方手机号</th>",'<td><input type="text" bid="" id="BMapLib_phone_0" maxlength="11" class="BMapLib_sms_input BMapLib_sms_input_l" /><span id="BMapLib_activateTip"></span></td>',"</tr>",'<tr id="BMapLib_activateBox" style="display:none;
	">',"<th>激活码</th>",'<td><input type="text" id="BMapLib_activate" class="BMapLib_sms_input BMapLib_sms_input_s" maxlength="4"/><input type="button" value="获取" id="BMapLib_activate_btn" bid="activate" />',"</tr>","<tr>","<th></th>","<td>","</td>","</tr>","<tr>",'<th style="vertical-align:top;
	padding-top:7px;
	">接收方手机号</th>','<td><div><input type="text" id="BMapLib_phone_1" class="BMapLib_sms_input BMapLib_sms_input_l" maxlength="11"/><input type="checkbox" id="BMapLib_is_remember_phone"/>记住此号</div>','<div id="BMapLib_add_phone_con"></div>','<div><a href="javascript:void(0)" id="BMapLib_add_phone_btn" bid="addPhone">新增</a></div>',"</td>","</tr>","<tr>","<th></th>",'<td><input type="button" value="免费发送到手机" bid="sendToPhoneBtn"/></td>',"</tr>","</table>","</div>",'<div class="BMapLib_mp" id="pnl_phone_right" style="display:block;
	">','<div class="BMapLib_mp_title">短信内容：</div>','<div id="BMapLib_msgContent" class="BMapLib_msgContent"></div>',"</div>",'<div style="clear:both;
	"></div>','<p id="BMapLib_sms_declare_phone" class="BMapLib_sms_declare_phone">百度保证不向任何第三方提供输入的手机号码</p>','<div id="tipError" class="tip fail" style="display:none;
	">','<span id="tipText"></span>','<a href="javascript:void(0)" id="tipClose" class="cut"></a>',"</div>",'<div id="sms_lack_tip" style="display:none;
	">已达每日5次短信上限</div>','<div id="sms_unlogin_tip" style="display:none;
	">','<div style="padding-left:40px;
	">','<ul class="login_ul"><li id="normal_login-2" class="login_hover"><a href="javascript:void(0)">手机登录</a></li></ul>','<div id="loginBox_02" class="loginBox">','<div id="pass_error_info-2" class="pass_error_style"></div>','<div id="passports-2"></div>','<div id="loginIframe_iph-2" style="display:none"></div>',"</div>","</div>",'<div class="nopass" style="padding-left:128px;
	">没有百度帐号？<a href="https://passport.baidu.com/v2/?reg&regType=1&tpl=ma" target="_blank">立即注册</a></div>',"</div>","</div>",'<button class="BMapLib_popup_close" bid="close"></button>','<div id="BMapLib_success_tip" style="display:none;
	">您的短信已经发送到您的手机，请注意查收!</div>',].join("");
	m.innerHTML=i;
	return m}
,initDom:function() {
	this.dom= {
	sms_tip:e.g("BMapLib_sms_tip"),activate_btn:e.g("BMapLib_activate_btn"),fromphone:e.g("BMapLib_phone_0"),tophone:e.g("BMapLib_phone_1"),isRememberPhone:e.g("BMapLib_is_remember_phone"),sms_container:e.g("BMapLib_sms_pnl_phone"),success_tip:e.g("BMapLib_success_tip"),add_phone_con:e.g("BMapLib_add_phone_con"),add_phone_btn:e.g("BMapLib_add_phone_btn"),activateBox:e.g("BMapLib_activateBox"),activateTip:e.g("BMapLib_activateTip"),activate_input:e.g("BMapLib_activate")}
}
,showTip:function(i) {
	var j=i.error;
	var m= {
	PHONE_NUM_INVALID:"手机号码无效",SMS_SEND_SUCCESS:"发送到手机成功",AK_INVALID:"你所使用的key无效",INTERNAL_ERROR:"服务器错误",OVER_MAX_ACTIVATE_TIME:"今天已超过发送激活码最大次数",SMS_ACTIVATE_SUCCESS:"激活码已发送到您的手机，请注意查收！",ACTIVATE_FAIL:"手机激活码无效",SMS_LACK:"今天您还能往5个手机发送短信",PARAM_INVALID:"请填完所有选项",SEND_ACTIVATE_FAIL:"激活码发送失败"}
;
	var l=m[j];
	if(j=="SMS_LACK") {
	var k=i.res_sms;
...
,bind:function() {
	...
	var m=n.target||n.srcElement,l=m.getAttribute("bid");
	switch(l) {
	case"close":k.closeActon();
	break;
	case"sendToPhoneBtn":k.sendAction();
	break;
	case"activate":k.activateAction();
	break;
	case"addPhone":k.addPhoneAction();
	break;
	case"deletePhone":k.deletePhoneAction(m);
	break}
}
);
	...
	var m=o||window.e,n=m.which||m.keyCode,l=false;
...
,checkActivateAction:function() {
	var j= {
	phone:this.dom.fromphone.value,activate:this.dom.activate_input.value,cbName:"callback"}
;
	var i=this;
...
,activateAction:function() {
	...
	var j= {
	phone:this.dom.fromphone.value,ak:k,cbName:"callback"}
;
...
var o=this._map.getKey();
	var n= {
	fromphone:e.g("BMapLib_phone_0").value,tophone:tophoneStr,ak:o,activate:this.dom.activate_input.value,content:e.g("BMapLib_phone_0").value+"分享一个位置给您，"+this.messageContent,cbName:"callback"}
;
...
,getAddressByPoint:function() {
	var k=this.iw._point,i=this,j=new BMap.Geocoder();
...
var l="http://api.map.baidu.com/marker?location="+i.lat+","+i.lng+"&title="+encodeURIComponent(j.getTitle())+"&content="+encodeURIComponent(j.getContent())+"&output=html";
	var m= {
	url:encodeURIComponent(l),t:new Date().getTime(),cbName:"callback"}
;
	var k=this;
...
,addPhoneAction:function() {
	...
	i.innerHTML='<input type="text" class="BMapLib_sms_input BMapLib_sms_input_l" maxlength="11"/><a href="javascript:void(0);
	" style="margin-left:5px;
	" bid="deletePhone">删除</a>';
...
var l=this;
	...
	b(k)}
,config: {
	sendURL:h+"/ws/message?method=send",activateURL:h+"/ws/message?method=activate",ckActivateURL:h+"/ws/message?method=ckActivate",shortURL:"http://j.map.baidu.com/?"}
}
);
	function b(j) {
