...
// 
// 
//Edit by 高振刚  at 20150422  gaozhengang@zhonghu.cn
namespace myvendor;
class AjaxPage {
    // 分页栏每页显示的页数
    public $rollPage = 5;
    ...
    protected $coolPages   ;
    // 分页显示定制
    protected $config  = array('header'=>'条记录','prev'=>'<','next'=>'>','first'=>'第一页','last'=>'最后一页','theme'=>' %totalRow% %header% %nowPage%/%totalPage% 页 %upPage%  %first%  %prePage%  %linkPage%  %nextPage% %end% %downPage%');
    // 默认分页变量名
    protected $varPage;
    ...
    public function __construct($totalRows,$listRows='',$ajax_func,$parameter='') {
        ...
        $this->varPage = config('var_page') ? config('var_page') : 'pageNum' ;
        if(!empty($listRows)) {
