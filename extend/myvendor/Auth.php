...
//数据库
/*
 * CREATE TABLE `guguo_auth_group` (
      `id` mediumint(8) unsigned NOT NULL AUTO_INCREMENT,
      `title` char(100) NOT NULL DEFAULT '' COMMENT '用户组中文名称',
      `status` tinyint(1) NOT NULL DEFAULT '1' COMMENT '用户组状态：为1正常，为0禁用,-1为删除',
      `rules` char(80) NOT NULL DEFAULT '' COMMENT '用户组拥有的规则id，多个规则 , 隔开',
      `type` tinyint(1) NOT NULL COMMENT '组类型',
      `description` varchar(45) DEFAULT NULL COMMENT '描述信息',
      PRIMARY KEY (`id`)
    ) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8
 *
 * CREATE TABLE `guguo_auth_group_access` (
...
//        $user = $this->getUserInfo($uid);//获取用户信息,一维数组
    ...
    protected function getUserInfo($uid)
    {
        ...
        if (!isset($userinfo[$uid])) {
            $userinfo[$uid] = $this->model->table($this->_config['AUTH_USER'])->where('uid', $uid)->find();
        }
        return $userinfo[$uid];
    }
