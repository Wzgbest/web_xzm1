...
function public_pool_previous_page(p,num){
	...
	public_pool_change_page(p-1,num);
}
function public_pool_next_page(p,num,max){
	if(p+1>max){
		return;
	}
	public_pool_change_page(p+1,num);
...
function public_pool_change_page(p,num){
	loadPage(get_public_pool_url(p,num),"high-seafr");
}
function public_pool_search(p,num){
	var url = get_public_pool_url(p,num);
	var take_type = $("#public_pool_search_take_type").val();
	if(take_type!=""){
		url += "/take_type/"+take_type;
	}
	var grade = $("#public_pool_search_grade").val();
	if(grade!=""){
		url += "/grade/"+grade;
	}
	var sale_chance = $("#public_pool_search_sale_chance").val();
	if(sale_chance!=""){
		url += "/sale_chance/"+sale_chance;
	}
	var comm_status = $("#public_pool_search_comm_status").val();
	if(comm_status!=""){
		url += "/comm_status/"+comm_status;
	}
	var customer_name = $("#public_pool_search_customer_name").val();
	if(customer_name!=""){
		url += "/customer_name/"+customer_name;
	}
	var contact_name = $("#public_pool_search_contact_name").val();
	if(contact_name!=""){
		url += "/contact_name/"+contact_name;
	}
	loadPage(url,"high-seafr");
}
function get_public_pool_url(p,num){
