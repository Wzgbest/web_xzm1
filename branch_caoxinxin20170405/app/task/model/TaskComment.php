...
class TaskComment extends Base{
    ...
    public function getAllTaskComment($task_ids,$map=null,$order="id desc"){
        ...
        $employeeTaskCommentList = $this->model->table($this->table)->alias('etc')
            ...
            ->where($map)
            ->order($order)
            ->field("etc.*,etc.replyer_id as replyer_id,etc.comment_time as commont_time,etc.reply_comment_id,rp.telephone as replyer_telephone,rp.truename as replyer_name,rp.userpic as replyer_pic,rv.telephone as reviewer_telephone,rv.truename as reviewer_name,rv.userpic as reviewer_pic")
            ->select();
